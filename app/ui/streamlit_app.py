...
def render_exports_tab(filtered: pd.DataFrame) -> None:
    """Minimalist exports tab — exposes only the two downloads buyers
    actually need : Companies (CSV + XLSX) and Attendance Signals
    (CSV + XLSX). Every other legacy / dashboard widget has been
    intentionally removed so the page reads as four clear buttons.

    Both sections are bilingual : column headers and labels switch
    between FR and EN with ?lang=en. The Companies download reflects
    the current sidebar filters (what-you-see-is-what-you-export) ;
    the Attendance download covers every signal that's been ingested
    so far (subject to the 50-row demo cap when in demo mode).
    """
    ...
    # ============================================================
    # 🏢 Companies — CSV + XLSX (mirrors the on-screen table, filtered)
    # ============================================================
    st.markdown(f"### {t('exports.companies.title')}")
    companies_export = visible_columns_only(filtered)
    n_rows = len(companies_export)
    st.caption(t("exports.companies.stats", total=n_rows))

    companies_cols = st.columns(2)
    with companies_cols[0]:
        st.download_button(
            t("exports.companies.csv_label"),
            data=companies_export.to_csv(index=False).encode("utf-8"),
            file_name=f"companies_{datetime.utcnow():%Y%m%d_%H%M%S}.csv",
            mime="text/csv",
            use_container_width=True,
            type="primary",
            help=t("exports.companies.csv_help"),
            key="exports_companies_csv",
        )
    with companies_cols[1]:
        buf_c = BytesIO()
        with pd.ExcelWriter(buf_c, engine="openpyxl") as w:
            companies_export.to_excel(w, index=False, sheet_name="Companies")
            ws = w.sheets["Companies"]
            if n_rows > 0 and len(companies_export.columns) > 0:
                from openpyxl.utils import get_column_letter
                last_col_letter = get_column_letter(len(companies_export.columns))
                ws.auto_filter.ref = f"A1:{last_col_letter}{n_rows + 1}"
        buf_c.seek(0)
        st.download_button(
            t("exports.companies.xlsx_label"),
            data=buf_c,
            file_name=f"companies_{datetime.utcnow():%Y%m%d_%H%M%S}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
            help=t("exports.companies.xlsx_help"),
            key="exports_companies_xlsx",
        )

    # ============================================================
    # 📡 Attendance signals — CSV + XLSX (prospection-ready)
    # ============================================================
    st.markdown("---")
    st.markdown(f"### {t('exports.attendance.section_title')}")
    ...
    n_with_email = (
        int(att_export.get(email_col, pd.Series()).notna().sum())
        if email_col in att_export.columns else 0
    )
    n_with_phone = (
        int(att_export.get(phone_col, pd.Series()).notna().sum())
        if phone_col in att_export.columns else 0
    )
    n_with_linkedin = (
        int(att_export.get("LinkedIn", pd.Series()).notna().sum())
        if "LinkedIn" in att_export.columns else 0
    )
    st.caption(
        t(
    ...
    with att_cols[1]:
        ...
        with pd.ExcelWriter(buf_att, engine="openpyxl") as w:
            ...
            if n_total > 0 and len(att_export.columns) > 0:
                from openpyxl.utils import get_column_letter
                last_col_letter = get_column_letter(len(att_export.columns))
                ws.auto_filter.ref = f"A1:{last_col_letter}{n_total + 1}"
        buf_att.seek(0)
        st.download_button(
...
def main() -> None:
    ...
    with tab_exports:
        # Topbar (quick CSV/XLSX/Dynamics/Salesforce/HubSpot/Reset) and
        # KPI cards intentionally removed — the Exports tab now exposes
        # ONLY four download buttons (Companies CSV/XLSX, Attendance
        # CSV/XLSX) so the buyer sees a single clean choice.
